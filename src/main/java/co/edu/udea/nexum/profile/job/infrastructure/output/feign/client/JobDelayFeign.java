...
import static co.edu.udea.nexum.profile.job.infrastructure.output.feign.utils.constants.JobFeignConstants.JOB_DELAY_FEIGN_NAME;
import static co.edu.udea.nexum.profile.job.infrastructure.output.feign.utils.constants.JobFeignConstants.JOB_DELAY_FEIGN_URL;

@FeignClient(
        name = JOB_DELAY_FEIGN_NAME,
        url = JOB_DELAY_FEIGN_URL,
        configuration = {FeignClientConfiguration.class, FeignErrorDecoder.class, FeignBasicInterceptor.class}
)
public interface JobDelayFeign {
    @GetMapping(COMMON_ID_PATH)
    JobDelayFeignResponse findById(@PathVariable Long id);
}
