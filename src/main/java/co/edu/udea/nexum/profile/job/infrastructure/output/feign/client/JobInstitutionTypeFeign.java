...
import static co.edu.udea.nexum.profile.job.infrastructure.output.feign.utils.constants.JobFeignConstants.JOB_INSTITUTION_TYPE_FEIGN_NAME;
import static co.edu.udea.nexum.profile.job.infrastructure.output.feign.utils.constants.JobFeignConstants.JOB_INSTITUTION_TYPE_FEIGN_URL;

@FeignClient(
        name = JOB_INSTITUTION_TYPE_FEIGN_NAME,
        url = JOB_INSTITUTION_TYPE_FEIGN_URL,
        configuration = {FeignClientConfiguration.class, FeignErrorDecoder.class, FeignBasicInterceptor.class}
)
public interface JobInstitutionTypeFeign {
    @GetMapping(COMMON_ID_PATH)
    JobInstitutionTypeFeignResponse findById(@PathVariable Long id);
}
